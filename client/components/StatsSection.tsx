      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            {isRTL() 
              ? "أرقام تتحدث عن التميز والثقة التي يضعها عملاؤنا في خدماتنا وجودة منتجاتنا"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-20">
          {stats.map((stat, index) => {
            ...
            return (
              ...
              >
                <div className="glass-card p-8 text-center premium-shadow hover:premium-shadow-xl transition-all duration-300 border-gradient">
                  {/* Animated Background */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${stat.gradient} opacity-0 group-hover:opacity-10 rounded-xl transition-opacity duration-300`}></div>
                  
                  {/* Icon */}
                  <motion.div
                    whileHover={{ rotate: 360, scale: 1.2 }}
                    transition={{ duration: 0.6 }}
                    className={`w-16 h-16 mx-auto mb-6 bg-gradient-to-br ${stat.gradient} rounded-xl flex items-center justify-center relative`}
                  >
                    <Icon className="h-8 w-8 text-white" />
                    <motion.div
                      animate={{ scale: [1, 1.2, 1] }}
                      transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                      className={`absolute inset-0 bg-gradient-to-br ${stat.gradient} rounded-xl blur-lg opacity-30`}
                    ></motion.div>
                  </motion.div>
